...
from numpy import array, random, stack
from numpy.random import normal
from tensorflow import keras, TensorShape, GradientTape, concat
...
class TwinDelayedDDPG:
    def __init__(self,
        ...
        q_input_shape = TensorShape([None, 1 + state_size])
        policy_input_shape = TensorShape([None, state_size])
    ...
    def get_action(self, state):
        ...
        policy_action = self.policy(array([state]))[0]
        return min(max(policy_action + normal(0, self.noise_std), array([-1])), array([1]))
        ...
        while True:
            ...
            if step_idx % self.steps_until_sync == 0 and len(self.replay_buffer) >= self.batch_size:
                ...
                with GradientTape() as q_tape:
                    ...
                    q_pred = self.q(concat([s_batch, a_batch], axis=1))

                    a_next_batch = self.policy_target(s_next_batch)
                    q_gt = r_batch + self.discount * self.q_target(concat([s_next_batch, a_next_batch], axis=1))
                ...
                with GradientTape() as policy_tape:
                    ...
                    policy_loss = -self.q(concat([s_batch, a_pred_batch], axis=1))
    ...
    def gridsearch(data):
        ...
        for hyperparams in _get_hyperparam_seq(hyperparam_names):
            ...
            try:
                ...
                agent = TwinDelayedDDPG(data_, **hyperparams)
